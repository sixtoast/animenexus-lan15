...
export function AddToWatchlist({ anime }: Props) {
  ...
  if (!entry) return <div className="wl-actions mobile-sticky-cta">
    <Button variant="accent" size="sm" onClick={()=>{const ok=add(anime,"planning");reactToOutcome(ok,"choice");if(ok)showToast("Sealed to your list","🕯️",true);else showToast(FAIL_MSG,"⚠️")}}>+ Add to watchlist</Button>
    <Button variant="outline" size="sm" onClick={()=>{const ok=add(anime,"watching");reactToOutcome(ok,"success");if(ok)showToast("Now watching","▶",true);else showToast(FAIL_MSG,"⚠️")}}>Start watching</Button>
  </div>;
  return <div className="wl-actions mobile-sticky-cta">
    <label className="wl-status-label"><span className="filter-label">List status</span><select className="filter-input" value={entry.watchStatus} onChange={(e)=>{const ok=setStatus(anime.id,e.target.value as WatchStatus);reactToOutcome(ok,"choice");if(!ok)showToast(FAIL_MSG,"⚠️")}}>{STATUSES.map(s=><option key={s.value} value={s.value}>{s.label}</option>)}</select></label>
    <Button variant="danger" size="sm" onClick={()=>{const ok=remove(anime.id);reactToOutcome(ok,"choice");if(ok)showToast("Removed from list","·");else showToast(FAIL_MSG,"⚠️")}}>Remove</Button>
  </div>;
}
