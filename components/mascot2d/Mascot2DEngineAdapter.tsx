...
export function Mascot2DEngineAdapter({className,depth=1,yaw=0,speed=0,justLanded=false,gazeOverride={x:0,y:0},perchPose="stand"}:Mascot2DEngineAdapterProps){
 ...
 // Tiny target noise cannot steal attention. Meaningful changes must persist briefly unless they are clearly more salient.
 ...
 // When a target disappears, keep it for a short beat instead of snapping immediately to neutral.
 if(incomingMag<.035&&focusMag>.12&&now<holdUntil.current)accepted.current=focus;
 // One restrained glance-back may occur after attention has moved away from an interesting target.
