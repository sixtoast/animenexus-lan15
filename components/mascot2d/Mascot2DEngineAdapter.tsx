...
export function Mascot2DEngineAdapter({className,depth=1,yaw=0,speed=0,justLanded=false,gazeOverride={x:0,y:0},perchPose="stand"}:Mascot2DEngineAdapterProps){
 ...
 return <LanternKo2D expression={expression} emotions={emotions} lookBias={lookBias} gazeOverride={continuousGaze} anim={anim} yaw={yaw} speed={speed} justLanded={justLanded} depth={depth} className={className} context={{intention,lastLandmarkType}} perchPose={perchPose} speaking={conversation.speaking} listening={conversation.listening&&!conversation.thinking} thinking={conversation.thinking&&!conversation.speaking}/>;
}
