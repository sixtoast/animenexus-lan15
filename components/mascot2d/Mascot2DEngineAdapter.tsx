...
import {useMascotConversationState} from "@/lib/mascot/conversation-state";
...
import {useThinkingActing2D} from "./useThinkingActing2D";
import type {PerchPose} from "./types";
import "./contextual-acting.css";
import "./thinking-acting.css";
...
export function Mascot2DEngineAdapter({className,depth=1,yaw=0,speed=0,justLanded=false,gazeOverride={x:0,y:0},perchPose="stand"}:Mascot2DEngineAdapterProps){
 const rawEmotions=useMascotStore(s=>s.emotions),emotions=useEmotionInertia2D(rawEmotions),anim=useMascotStore(s=>s.anim),layers=useMascotStore(s=>s.layers),lookBias=useMascotStore(s=>s.lookBias),intention=useMascotStore(s=>s.intention),lastLandmarkType=useMascotStore(s=>s.lastLandmarkType),conversation=useMascotConversationState(),expression=faceForActor(anim,emotions,layers.social!=="none");
 const thinkingActive=conversation.thinking&&!conversation.speaking,thinking=useThinkingActing2D({active:thinkingActive,curiosity:emotions.curiosity,attention:emotions.attention,energy:emotions.energy,sleepiness:emotions.sleepiness,stress:emotions.stress});
 ...
 const thinkingGaze={x:clamp(continuousGaze.x+thinking.gazeX),y:clamp(continuousGaze.y+thinking.gazeY)},thinkingStyle={"--think-tilt":`${thinking.tilt}deg`,"--think-nod":`${thinking.nod}px`,"--think-lean":thinking.lean,"--think-tilt-n":thinking.tilt,"--think-nod-n":thinking.nod} as CSSProperties;
 return <span data-mascot-thinking={thinkingActive?"true":"false"} data-mascot-think-act={thinking.kind} style={thinkingStyle}><LanternKo2D expression={expression} emotions={emotions} lookBias={lookBias} gazeOverride={thinkingGaze} anim={anim} yaw={yaw} speed={speed} justLanded={justLanded} depth={depth} className={className} context={{intention,lastLandmarkType}} perchPose={perchPose} speaking={conversation.speaking} listening={conversation.listening&&!conversation.thinking} thinking={thinkingActive}/></span>;
}
