...
/** Render-only bridge. New image-first art is preferred; the proven SVG puppet remains an automatic fallback. */
export function Mascot2DEngineAdapter({className,depth=1,yaw=0,speed=0,justLanded=false,gazeOverride={x:0,y:0},perchPose="stand"}:Mascot2DEngineAdapterProps){
 const emotions=useMascotStore(s=>s.emotions),anim=useMascotStore(s=>s.anim),layers=useMascotStore(s=>s.layers),lookBias=useMascotStore(s=>s.lookBias),intention=useMascotStore(s=>s.intention),lastLandmarkType=useMascotStore(s=>s.lastLandmarkType),expression=faceForActor(anim,emotions,layers.social!=="none"),[artFailed,setArtFailed]=useState(false);
 ...
 if(!artFailed)return <LiveArtLanternKo emotions={emotions} anim={anim} yaw={yaw} speed={speed} justLanded={justLanded} gazeOverride={continuousGaze} perchPose={perchPose} onAssetError={()=>setArtFailed(true)}/>;
