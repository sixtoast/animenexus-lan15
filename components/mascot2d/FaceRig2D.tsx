...
type Props={expression:string;blink:boolean;gazeX:number;gazeY:number;blush:number;turn?:number};
const HAPPY=new Set(["happy","excited","proud","smug","mischievous"]);
const clamp=(v:number,lo=-1,hi=1)=>Math.min(hi,Math.max(lo,v));
export function FaceRig2D({expression,blink,gazeX,gazeY,blush,turn=0}:Props){
 ...
 const gx=clamp(gazeX/6),gy=clamp(gazeY/5),t=clamp(turn,-.6,.6),amount=Math.abs(t)/.6,squint=angry?.16:sleepy?.34:smug?.12:0,openness=blink?0:sleepy?.26:surprised?1.12:1-squint;
 const vars={"--fx":`${gazeX}px`,"--fy":`${gazeY}px`,"--fb":blush,"--eye-open":openness,"--pupil-x":`${gx*1.4}px`,"--pupil-y":`${gy*.9}px`,"--iris-scale":surprised?.94:angry?1.04:1,"--lid-bias":`${gy*1.5}px`,"--turn":t,"--turn-amount":amount,"--feature-shift":`${t*5.2}px`,"--nose-turn":`${t*7.2}px`,"--mouth-turn":`${t*4.6}px`} as CSSProperties;
