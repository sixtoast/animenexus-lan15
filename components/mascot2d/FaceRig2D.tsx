...
export function FaceRig2D({expression,blink,gazeX,gazeY,blush,turn=0,mouthOpen=0,mouthWide=0,mouthMood=0}:Props){
 ...
 const gx=clamp(gazeX/6),gy=clamp(gazeY/5),t=clamp(turn,-.6,.6),amount=Math.abs(t)/.6,squint=angry?.16:sleepy?.34:smug?.12:0,openness=blink?0:sleepy?.26:surprised?1.12:1-squint,proceduralOpen=clamp(Math.max(surprised?.88:0,mouthOpen),0,1),proceduralWide=clamp(mouthWide,0,1),mood=clamp(mouthMood);
