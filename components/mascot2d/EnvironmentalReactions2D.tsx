 ...
 useEffect(()=>{if(!enabled)return;let timer=0;
  ...
  const inspect=()=>{try{refreshLandmarkRects();const current=new Map<string,Snapshot>(),landmarks=listLandmarks(),now=performance.now();for(const [id,m] of attention.current)if(now-m.seenAt>MEMORY_MS)attention.current.delete(id);for(const lm of landmarks){const s=snapshot(lm);if(s)current.set(lm.id,s)}if(!booted.current){previous.current=current;booted.current=true;return}
   ...
   if(!busy&&now-lastNoticeAt.current>1400){let candidate:{lm:Landmark;cue:EnvironmentalCue;distance:number;score:number}|null=null;
    ...
    else{const c=cursor.current,userActive=c&&now-c.at<USER_IDLE_MS;if(userActive&&c&&now-lastUserCueAt.current>3000){let focus:{lm:Landmark;cursorDistance:number;mascotDistance:number}|null=null;for(const lm of landmarks){const s=current.get(lm.id);if(!s||!REACTIVE.has(lm.type))continue;const cursorDistance=Math.hypot(s.cx-c.x,s.cy-c.y);if(cursorDistance>Math.max(120,Math.min(260,Math.hypot(s.width,s.height)*.65)))continue;const mascotDistance=Math.hypot(s.cx-mascot.clientX,s.cy-mascot.clientY);if(mascotDistance>620)continue;if(!focus||cursorDistance<focus.cursorDistance)focus={lm,cursorDistance,mascotDistance}}if(focus){lastUserCueAt.current=now;lastGlanceAt.current=now;emit("glance-back",focus.lm,focus.mascotDistance)}}
