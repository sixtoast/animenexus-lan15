...
export function useSpeechMouth2D({active,energy,happiness,stress,sleepiness}:Input):SpeechMouth{
 ...
 useEffect(()=>{let alive=true,timer=0,current=REST;const choose=()=>{if(!alive)return;const i=input.current;if(!i.active){current={open:0,wide:0,mood:clamp(i.happiness-i.stress*.45,-1,1),shape:"silence"};setMouth(current);timer=window.setTimeout(choose,180);return}const pauseChance=.13+i.sleepiness*.08;if(Math.random()<pauseChance){current={open:0,wide:.05,mood:clamp(i.happiness-i.stress*.4,-1,1),shape:"closed"};setMouth(current);timer=window.setTimeout(choose,75+Math.random()*150);return}const total=SHAPES.reduce((n,s)=>n+s.w,0),r=Math.random()*total;let acc=0,pick=SHAPES[0];for(const s of SHAPES){acc+=s.w;if(r<=acc){pick=s;break}}const intensity=.72+i.energy*.22-i.sleepiness*.12,stressTight=1-i.stress*.18;current={open:clamp(pick.open*intensity),wide:clamp(pick.wide*stressTight+i.happiness*.1),mood:clamp(i.happiness-i.stress*.4,-1,1),shape:pick.shape};setMouth(current);const cadence=82-i.energy*18+i.sleepiness*32+Math.random()*55;timer=window.setTimeout(choose,Math.max(58,cadence))};choose();return()=>{alive=false;clearTimeout(timer)}},[]);
 return mouth;
}
