...
import {LiveArtFaceRig} from "./LiveArtFaceRig";
...
/** Image-first renderer. V2 adds a continuous facial control plane over the coherent approved raster; SVG remains fallback upstream. */
export function LiveArtLanternKo({emotions,anim,yaw=0,speed=0,justLanded=false,gazeOverride={x:0,y:0},perchPose="stand",onAssetError}:LiveArtLanternKoProps){
 ...
 const vars={"--lk-yaw":`${yaw*5}deg`,"--lk-gaze-x":`${gazeOverride.x*1.2}px`,"--lk-gaze-y":`${gazeOverride.y*.8}px`,"--lk-speed":String(speed)} as React.CSSProperties;
 return <div className={styles.root} data-ready={ready} data-anim={anim} data-mood={mood} data-perch={perchPose} data-landed={justLanded||undefined} style={vars} aria-hidden>
  <div className={styles.shadow}/><div className={styles.breath}><div className={styles.turn}><div className={styles.artStack}><img className={styles.art} src={BASE_ART} alt="" draggable={false} onLoad={()=>setReady(true)} onError={onAssetError}/><LiveArtFaceRig emotions={emotions} anim={anim} gaze={gazeOverride} yaw={yaw}/></div></div></div>
 </div>;
}
