...
export function MascotHost(){
 ...
 function hideCompanion(){setEnabled(false);try{localStorage.setItem("anime_nexus_mascot","off")}catch{}setStatusMsg("Companion hidden. Site works without it.")}
 function showCompanion(){setEnabled(true);try{localStorage.setItem("anime_nexus_mascot","on")}catch{}setStatusMsg("Companion shown.")}
 function toggleAudio(){const next=!isAudioEnabled();setAudioEnabled(next);setAudioOn(next);setStatusMsg(next?"Companion sound on":"Companion sound off")}
 function toggleInteract(){const next=!areInteractionsEnabled();setInteractionsEnabled(next);setInteractOn(next);setStatusMsg(next?"Companion interactions on":"Companion interactions off")}
 function chooseRenderer(next:MascotRenderer){setRenderer(next);try{localStorage.setItem(RENDERER_KEY,next)}catch{}setStatusMsg(next==="2.5d"?"Lantern-ko switched to 2.5D":"Lantern-ko switched to 3D")}
 useEffect(()=>{if(!enabled)return;return bindMascotKeyboard({toggleHide:hideCompanion,toggleMute:toggleAudio,toggleInteractions:toggleInteract})},[enabled]);
 ...
 if(webglError&&renderer==="3d")renderer="2.5d";
