...
export function MascotHost(){
 ...
 const chooseRenderer=useCallback((next:MascotRenderer)=>{if(next==="3d"&&webglError){setStatusMsg("3D is unavailable here. Lantern-ko is staying in 2.5D.");return}setRenderer(next);try{localStorage.setItem(RENDERER_KEY,next)}catch{}setStatusMsg(next==="2.5d"?"Lantern-ko switched to 2.5D":"Lantern-ko switched to 3D")},[webglError]);
