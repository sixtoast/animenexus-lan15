...
export function PlaceholderChibi() {
  ...
  const lastStorePos = useRef({ x: 0, y: 0 });
  ...
  useFrame((_state, delta) => {
    ...
    if (
      ...
      Math.abs(store.position.y - lastStorePos.current.y) > 0.15
    ) {
      if (!dragging.current) {
    ...
    lastStorePos.current = { x: store.position.x, y: store.position.y };
    ...
    setPosition({ x: body.current.x, y: body.current.z });
    ...
    <group
      ...
      onPointerMove={(e) => {
        if (dragging.current) {
          ...
          useMascotStore.getState().setPosition({ x, y: z });
        }
      }}
      onPointerDown={(e) => {
        e.stopPropagation();
